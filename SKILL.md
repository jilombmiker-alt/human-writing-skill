---
name: human-writing
description: 通用中文创作与改稿 Skill。用于知乎回答、论坛长帖、公众号文章、博客、评论、人物故事、历史叙事、新闻与行业解读、AI 产品文章的选题与写作、科普、教程、评测、个人叙事、小说、故事、对白、口播和演讲稿。AI 产品文章先推荐 3 到 5 个写作方向，覆盖三天内的小众前沿技术、AI 工具对比与适用场景、特定技术领域的深度观点，每个方向提供 3 到 5 条经打开核验的参考链接。确认方向后再补充同类文章，等用户审阅并回传选定材料后写作。公众号稿件含图表时，同时交付可复制正文、逐张独立图片、插入位置和图注，不能只把图片嵌在文档里。默认写成一个见过事、查过材料、愿意把来龙去脉讲清楚的人在说话，重点保留中文互联网长回答与长帖的活人感和自然中文韵律，避免空泛的机构腔、喊口号式演说腔、营销腔和模型腔。非虚构长文先检查材料够不够，材料不足时研究、追问或缩短，绝不用重复解释灌字数。现实内容额外核验事实、引语、数据与用户亲历，虚构内容可以创造人物、场景、对白、心理与情节。成稿正文严禁冒号、破折号、“不是……而是……”及同类翻案句，并清除商业黑话和模型惯用黑话。不创建作者画像、个人规则库或个人写作 Skill。
---
...
- 用户想写 AI 产品、AI 应用、Agent、模型商业化或 AI 行业观察类文章，先读 `references/ai-product-research.md`。在用户确认方向并回传选定材料前，停在推荐或材料确认阶段，不起草正文。
...
用户要求公众号排版、图表、配图或可直接放进草稿的模板时，正文和图片按两条独立路径交付。

- 图表只放在能够解释数据、过程、对比或决策的位置。图中的数字、比例和来源必须与正文一致，不凭空补数据。
- 同一篇文章的图片统一画幅、颜色、字体、留白和图注格式。需要连续阅读时，优先使用相同宽高比和编号规则。
- 可以把图片嵌进 Word、HTML 或其他正文模板，嵌入版本不能成为唯一交付。公众号编辑器可能无法从其他应用稳定复制图片。
- 每张图必须另存为独立图片文件，按正文顺序编号并逐张提供。文件名写清内容，例如 `01_多Agent问题总览.png`。
- 同时给出插入清单。每项写图片编号、插在正文哪一句或哪一段之后、推荐图注和数据来源。插入位置使用正文里能直接搜索的完整句子，不能只写“放在第二部分”。
- 用户要求直接复制到公众号草稿时，明确说明操作顺序。先复制正文，再按插入清单逐张上传图片。不能承诺 Word 或 HTML 会把图片稳定带进公众号。
- 创建了可复用文件时，正文模板和所有独立图片都要保存并交付。用户没有要求压缩包时，不额外打包。
